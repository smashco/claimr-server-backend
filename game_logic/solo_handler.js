...
async function handleSoloClaim(io, socket, player, players, trail, baseClaim, client) {
    ...
    if (otherIntersectingIds.length === 0 && !isInitialBaseClaim) {
        ...
        const unionResult = await client.query(`SELECT ST_Union($1::geometry, ${newAreaWKT}) AS final_area`, [attackerExistingAreaRes.rows[0].area]);
        const finalGeom = `ST_MakeValid('${unionResult.rows[0].final_area}')`;
        const finalAreaResult = await client.query(`SELECT ST_AsGeoJSON(${finalGeom}) as geojson, ST_Area(${finalGeom}::geography) as area_sqm`);
    ...
    for (const victim of directHitVictimsResult.rows) {
        ...
        if (victim.is_shield_active) {
            ...
            const remainingSqM = turf.area(JSON.parse(remainingResult.rows[0].remaining_geojson) || turf.polygon([]));
            if (remainingSqM < 1) {
