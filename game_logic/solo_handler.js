...
async function handleSoloClaim(io, socket, player, players, trail, baseClaim, client) {
    ...
    if (isInitialBaseClaim) {
        ...
        if (intersectionResult.rowCount > 0) {
            console.log(`[DEBUG] Base claim rejected: inside existing territory.`);
        ...
        try {
            ...
            console.error(`[ERROR] Invalid base circle geometry: ${e.message}`);
            socket.emit('claimRejected', { reason: 'Invalid base location geometry.' });
            return null;
        }

        newAreaSqM = turf.area(newAreaPolygon);
        console.log(`[DEBUG] New base area: ${newAreaSqM.toFixed(2)} sqm`);

    } else {
        if (trail.length < 3) {
            console.log(`[DEBUG] Claim rejected: trail has less than 3 points`);
        ...
        try {
            ...
            console.error(`[ERROR] Invalid trail polygon geometry: ${e.message}`);
        ...
        newAreaSqM = turf.area(newAreaPolygon);
        if (newAreaSqM < 100) {
            console.log(`[DEBUG] Claim rejected: area too small (${newAreaSqM.toFixed(2)} sqm)`);
        ...
        if (!existingAreaGeoJSON || turf.area(JSON.parse(existingAreaGeoJSON)) === 0) {
            console.log(`[DEBUG] Claim rejected: no existing base for expansion`);
        ...
        console.log(`[DEBUG] Expansion intersects existing base: ${intersectsExisting.rows[0].intersects}`);
    ...
    let attackerNetGainGeom = geomResult.rows[0].geom;
    ...
    console.log(`[DEBUG] Found ${intersectingTerritoriesResult.rowCount} intersecting opponents`);
    ...
    for (const row of intersectingTerritoriesResult.rows) {
        ...
        if (row.is_shield_active) {
            console.log(`[GAME] Shield active for ${row.username} (${victimId}) — creating island`);
            ...
            console.log(`[GAME] Wiping out unshielded player: ${row.username} (${victimId})`);
    ...
    if (!finalAreaGeoJSON || finalAreaSqM < 1) {
        console.warn(`[WARN] Claim was nullified after attack handling. Final Area = ${finalAreaSqM}`);
        socket.emit('claimRejected', { reason: 'Claimed area was nullified by protected territories.' });
        return null;
    }

    const updateQuery = `UPDATE territories SET area = ST_GeomFromGeoJSON($1), area_sqm = $2 WHERE owner_id = $3;`;
    await client.query(updateQuery, [finalAreaGeoJSON, finalAreaSqM, userId]);
    console.log(`[SUCCESS] Territory updated for ${player.name} (${userId}) — New Total Area: ${finalAreaSqM.toFixed(2)} sqm`);
    ...
    if (isInitialBaseClaim) {
        ...
        console.log(`[DEBUG] Initial base point stored`);
    }
