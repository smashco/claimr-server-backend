...
async function handleSoloClaim(io, socket, player, players, trail, baseClaim, client) {
    ...
    if (isInitialBaseClaim) {
        ...
        if (isInfiltrator) {
            ...
            // Use robust insert/update for the attacker
            const saveQuery = `
    ...
    // --- Part 3: Shared Logic for Regular Base & Expansion ("Net Gain" Method) ---
    console.log(`[DEBUG] SECTION 2: Calculating Net Gain...`);
    const newAreaWKT = `ST_MakeValid(ST_GeomFromGeoJSON('${JSON.stringify(newAreaPolygon.geometry)}'))`;
    ...
    for (const row of intersectingTerritoriesResult.rows) {
        affectedOwnerIds.add(row.owner_id);
        if (row.is_shield_active) {
            console.log(`[DEBUG]   - Victim ${row.username} is SHIELDED. Punching hole in net gain.`);
            await client.query('UPDATE territories SET is_shield_active = false WHERE owner_id = $1', [row.owner_id]);
